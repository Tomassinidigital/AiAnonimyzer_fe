...
- Nell'**Anteprima PDF**, un clic su un punto libero della pagina apre la modale
  **"aggiungi frasi da offuscare"**: rilegge dal server il contenuto di quella
  sola pagina, mostra le frasi già offuscate dentro box del colore della gravità
  e permette di selezionarne di nuove; alla conferma le salva e riapplica subito
  le offuscazioni a tutte le viste.
...
- `POST /api/pdf/upload`, `GET /api/pdf/job/{id}`, `GET /api/pdf/layout/{doc_id}`,
  ...
  `GET /api/pdf/page/{doc_id}/{page}`, `GET /api/pdf/raw/{doc_id}`,
  `GET /api/pdf/download/{doc_id}/redact`
- `GET /healthz`
