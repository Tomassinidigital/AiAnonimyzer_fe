...
import { correctionParams, correctionQuery, type Range } from "@/utils/corrections";
import { PageObscureModal } from "@/components/modals/PageObscureModal";
...
export function PdfView() {
  ...
  // Pagina di cui è aperta la modale "aggiungi frasi da offuscare" (null = chiusa).
  const [obscurePage, setObscurePage] = useState<number | null>(null);
  // Incrementato per richiedere un ricarico completo (overlay + altre viste).
  ...
  // Correzioni manuali correnti, nel formato query degli endpoint PDF.
  const corrections = useMemo(
    () => correctionParams(manualRanges, excludedRanges, severities, allSeverities),
    [manualRanges, excludedRanges, severities, allSeverities],
  );
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const ov = await getPdfOverlay(docId, { ...corrections, mode: offuscMode });
      if (!pdfRef.current) {
  ...
  // Ricarico completo su richiesta (conferma della modale, ripristino da lista):
  // passa da un contatore perché `reload` va invocata DOPO che le nuove
  // correzioni sono nello store, altrimenti rileggerebbe quelle vecchie. Il
  // riferimento è tenuto aggiornato da un effetto dichiarato PRIMA, così allo
  // stesso commit punta già alla `reload` che vede le correzioni nuove.
  ...
  useEffect(() => {
    ...
    void (async () => {
      // Smaltisci il vecchio proxy pdf.js PRIMA di rifare il fetch: non
      // distruggerlo lascia worker orfani e le pagine successive vengono
      // renderizzate vuote (cfr. frontend originale).
      ...
      // Allinea anche report/anonimizzato/json alle correzioni manuali e ai
      // livelli di gravità correnti (il PDF non è l'unica vista).
      applyReRender();
    })();
  }, [refreshTick]);
  ...
  /** Conferma della modale: salva le nuove frasi e riapplica le offuscazioni. */
  function onObscureConfirm(ranges: Range[]) {
    ranges.forEach((r) => addManualRange(r));
    setObscurePage(null);
    if (ranges.length) {
      message.success(
        ranges.length === 1
          ? "1 nuova frase da offuscare salvata."
          : `${ranges.length} nuove frasi da offuscare salvate.`,
      );
    }
    setRefreshTick((t) => t + 1);
  }
  ...
  async function onDownload(kind: "box" | "mask") {
    ...
    const corr = correctionQuery(manualRanges, excludedRanges, severities, allSeverities);
    ...
    <div id="pdf-container-root">
      <div className="pdf-toolbar">
        ...
        {!scannedMsg && (
          <div className="pdf-nav-group">
            <span className="pdf-nav-hint">
              Clicca un punto libero della pagina per aggiungere frasi da offuscare
            </span>
          </div>
        )}
        {!scannedMsg && (
        ...
        <div className="pdf-body">
          <div className="pdf-container" ref={pdfContainerRef}>
            <div
              ...
              // Click su un punto LIBERO della pagina (i box già offuscati
              // fermano la propagazione): apre la modale per aggiungere frasi.
              onClick={() => setObscurePage(currentPage)}
              title="Clicca per aggiungere frasi da offuscare in questa pagina"
            >
      ...
      {obscurePage !== null && (
        <PageObscureModal
          ...
          page={obscurePage}
          params={corrections}
          onCancel={() => setObscurePage(null)}
          onConfirm={onObscureConfirm}
        />
      )}
    </div>
  );
